...
class ActorNetwork(object):
    ...
    def __init__(self, sess, state_dim, action_dim, action_bound, number_of_robots, message_length, learning_rate, tau, batch_size):
        self.sess = sess
    ...
    def create_actor_network(self):
        ...
        for i in range(self.num_robot):
            ...
            with tf.variable_scope(tag):
                mid_layer, msg = self.create_actor_unit_network_first_half(inputs)
                mid_layers.append(mid_layer)
                msgs.append(msg)

        msgs_layer = tf.concat(msgs, axis=1)

        for i in range(self.num_robot):
            tag = 'agent_' + str(i)
            with tf.variable_scope(tag):
                unit_out, unit_scaled_out = self.create_actor_unit_network_second_half(mid_layers[i], msgs_layer)
    ...
    def create_actor_unit_network_first_half(self, inputs):
        ...
        # Message Signal
        W_msg = tf.Variable(tf.truncated_normal([fc1_num_neurons, self.message_length], stddev=0.02), name='W_msg')
        b_msg = tf.Variable(tf.constant(0., shape=[self.message_length]), name='b_mgs')
        msg = tf.nn.tanh(tf.matmul(out_fc1, W_msg) + b_msg, name='msg')

        return out_fc1, msg

    def create_actor_unit_network_second_half(self, out_fc1, msgs):
        # Fully Connected Layer 2
        fc2_num_neurons = 300
        W_fc2 = tf.Variable(tf.truncated_normal([out_fc1.get_shape().as_list()[1], fc2_num_neurons], stddev=0.02), name='W_fc2')
        ...
        W_msgs = tf.Variable(tf.truncated_normal([self.num_robot*self.message_length, fc2_num_neurons], stddev=0.02), name='W_msgs')
        self.summary_list.append(tf.summary.histogram("W_msg_recv", W_msgs))
        out_fc2 = tf.nn.relu(tf.matmul(out_fc1, W_fc2) + tf.matmul(msgs, W_msgs) + b_fc2, name='out_fc2')
